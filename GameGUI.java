...
public class GameGUI extends JFrame {
    ...
    private transient final EnhancedGameImageManager imageManager;
    ...
    public GameGUI() {
        ...
        this.imageManager = EnhancedGameImageManager.getInstance();
    ...
    private void displayResult(Move playerMove, Move computerMove, GameResult result) {
        ...
        // Set result text with custom message
        if (gameType == GameType.PVP) {
            resultLabel.setText(result.getCustomMessage(player.getName(), player2.getName()));
        } else {
            resultLabel.setText(result.getCustomMessage("You", "Computer"));
        }
          // Create animated result display
        javax.swing.Timer resultTimer = new javax.swing.Timer(50, null);
        final int[] fontSize = {18}; // Start with default size
        final int maxSize = 32; // Maximum font size for animation
        final Font originalFont = resultLabel.getFont();
        
        resultTimer.addActionListener(e -> {
            if (fontSize[0] < maxSize) {
                fontSize[0] += 2;
                resultLabel.setFont(new Font("Arial", Font.BOLD, fontSize[0]));
            } else {
                resultTimer.stop();
                  // After a delay, restore original font
                javax.swing.Timer restoreTimer = new javax.swing.Timer(800, restore -> {
                    resultLabel.setFont(originalFont);
                });
    ...
    private void showGameHistory() {
        ...
        historyPane.setEditable(false);        // Green background with white text theme for game history
        Color historyBgColor = new Color(40, 160, 40); // Green background
        historyPane.setBackground(historyBgColor); 
        historyPane.setForeground(Color.WHITE); // White text for contrast
        
        // Attractive font with smaller text size
        Font historyFont = new Font("Segoe UI", Font.PLAIN, 13); // More attractive font, smaller size
        ...
        historyPane.setText(gameHistory.getFormattedHistory());        JScrollPane scrollPane = new JScrollPane(historyPane);
        scrollPane.setPreferredSize(new Dimension(600, 400));
        scrollPane.getViewport().setBackground(new Color(40, 160, 40)); // Match green background
    ...
    private void handleKeyPress(int keyCode) {
      ...
      private void startCountdown() {
        ...
        // Create large countdown font
        Font originalFont = resultLabel.getFont();
        Font countdownFont = new Font("Arial", Font.BOLD, 72);
        resultLabel.setFont(countdownFont);
        
        // Display countdown message with animation
        resultLabel.setText("" + countdownValue);
        
        // Show keybinding reminder
        String keybindMessage;
        if (gameType == GameType.PVP) {
            keybindMessage = String.format(
                "Player 1 (%s): A=Rock, S=Paper, D=Scissors | Player 2 (%s): J=Rock, K=Paper, L=Scissors",
                player.getName(), player2.getName()
            );
        } else {
            keybindMessage = "Keys: A=Rock, S=Paper, D=Scissors";
        }
        ...
        countdownTimer.addActionListener(e -> {
            ...
            if (countdownValue > 0) {
                ...
                resultLabel.setForeground(new Color(255, 0, 0)); // Red
                resultLabel.setText("" + countdownValue);
                soundManager.playSound("button_click"); // Click sound for countdown
                  // Flash effect
                javax.swing.Timer flashTimer = new javax.swing.Timer(500, flash -> {
                ...
                // Add final animation
                resultLabel.setForeground(new Color(0, 255, 0)); // Green
                resultLabel.setText("GO!");
                soundManager.playSound("button_click");
                  // Restore original font after a brief delay
                javax.swing.Timer restoreTimer = new javax.swing.Timer(500, restore -> {
                    resultLabel.setFont(originalFont);
                    resultLabel.setForeground(textColor);
                    resultLabel.setText("Processing result...");
    ...
    private void processRoundResult() {
        ...
        if (gameType == GameType.PVP) {
            ...
            if (player.hasCheated() && !player2.hasCheated()) {
                ...
                // Enhanced cheating notification with animation
                resultLabel.setFont(new Font("Arial", Font.BOLD, 24));
                resultLabel.setForeground(Color.RED);                resultLabel.setText("⚠️ " + player.getName() + " CHEATED! " + player2.getName() + " wins this round! ⚠️");
                soundManager.playSound("lose");
                
                // Create a flash animation for the cheating notification
                javax.swing.Timer flashTimer = new javax.swing.Timer(300, new ActionListener() {
                    boolean isRed = true;
                    int count = 0;
                ...
                // Enhanced cheating notification with animation
                resultLabel.setFont(new Font("Arial", Font.BOLD, 24));
                resultLabel.setForeground(Color.RED);                resultLabel.setText("⚠️ " + player2.getName() + " CHEATED! " + player.getName() + " wins this round! ⚠️");
      ...
      private void endMatch() {
        ...
        double player1WinPercentage = (double) scoreBoard.getPlayerWins() / totalRounds * 100;
        double player2WinPercentage = (double) scoreBoard.getComputerWins() / totalRounds * 100;
        double drawPercentage = (double) scoreBoard.getDraws() / totalRounds * 100;
    ...
    // Getter and setter methods for image icons
    public ImageIcon getRockIcon() {
        return rockIcon;
    }
    
    public void setRockIcon(ImageIcon rockIcon) {
    ...
    public void setPaperIcon(ImageIcon paperIcon) {
    ...
    public void setScissorsIcon(ImageIcon scissorsIcon) {
        this.scissorsIcon = scissorsIcon;
    }
    ...
    public void setLizardIcon(ImageIcon lizardIcon) {
        this.lizardIcon = lizardIcon;
    }
    ...
    public void setSpockIcon(ImageIcon spockIcon) {
        this.spockIcon = spockIcon;
    }
    
    // Additional getter methods for children classes to access
    ...
    // Protected getter methods for labels
    protected JLabel getPlayerScoreLabel() {
        return playerScoreLabel;
    }
    
    protected JLabel getComputerScoreLabel() {
        return computerScoreLabel;
    }
    
    protected JLabel getDrawsLabel() {
        return drawsLabel;
    }
    ...
    protected JLabel getPlayerMoveLabel() {
        return playerMoveLabel;
    }
    
    protected JLabel getComputerMoveLabel() {
